...
public class CLIBorne {
    ...
    private void depart(Long usager, Long idStationDepart, ArrayList<StationExport> stationsArrivee) throws QuaiInexistant, QuaiIndisponible, TempsTrajetInconnu, UtilisateurInconnu, StationInconnu, NavetteIndisponible {
        List<VoyageExport> vPrevus = this.serviceUsager.obtenirVoyagesPrevusUsager(usager);
        Calendar currDate = Calendar.getInstance();
        for (VoyageExport v : vPrevus){
            StationExport stationDepart = this.serviceUsager.obtenirStationParIdQuai(v.getQuaiDepart());
            if (v.getDateDepart().after(currDate) && stationDepart.getId().equals(idStationDepart)){
                StationExport stationArrivee = this.serviceUsager.obtenirStationParIdQuai(v.getQuaiArrivee());
                this.serviceUsager.reserverVoyage(usager, idStationDepart, stationArrivee.getId(), v.getNbPassagers(), v.getDateDepart());
                break;
            }
        }
        ...
        VoyageExport voyage = this.serviceUsager.reserverVoyage(usager, idStationDepart, idStationArrivee, (int) (long) nbPassagers, currDate);
